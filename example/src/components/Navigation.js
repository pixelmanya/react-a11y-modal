...
const NavTree = ({ children = NavItems, level = 1 }) => {
  return (
    <ul className={`NavigationLinks NavigationLinks--level-${level}`}>
      {children.map((node, idx) => (
        <li key={idx} className="NavigationLinks__item">
          <a
          ...
          {node.children ? (
            <NavTree children={node.children} level={level + 1} />
          ) : null}
        </li>
      ))}
    </ul>
